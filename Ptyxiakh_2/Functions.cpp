...
void populate_dispatchers()
{
    ...
    if(myfile.is_open()){
        myfile << "Simulation on Initialization\n";
        myfile << "Simulation created " << dispNumb <<" dispatchers successfully\n";
        myfile.close();
    }
}
...
void populate_cores()
{
    ...
    if(myfile.is_open()){
        myfile << "Simulation created " << coreNumb <<" cores successfully\n";
        myfile.close();
    }
}
...
int random_core()
{
    std::uniform_int_distribution<int> distribution(0,vCore.size() - 1);
    auto test = distribution(generator);
